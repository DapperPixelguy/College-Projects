...
def calculate_daily_food_costs(size, food, food_qty, active):
    ...
    base_cost = per_100g * (qty / 100)/100
...
def calculate_final_costs(daily_cost, vax):
    ...
    print("It is estimated that food for this dog will cost £{} \nper month ".format(format(est_monthly_food, '.2f')))

    print("It is estimated that food for this dog will cost £{} per year ".format(format(est_yearly_food, '.2f')))

    print(f'Yearly subtotal: £{format(est_yearly_food, '.2f')}')
    print("It is estimated that total cost for this dog (including vaccination) will be £{} per year ".format(format(
        total_inc_vax, '.2f')))
